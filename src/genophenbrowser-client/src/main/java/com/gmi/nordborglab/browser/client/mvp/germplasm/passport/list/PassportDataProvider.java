...
public class PassportDataProvider extends AsyncDataProvider<PassportProxy> implements HasHandlers {
    ...
    int start = 0;
    private PassportListPresenter.PassportProxyFilter passportProxyFilter;
    ...
    private void updateView() {
        ...
        if (filteredView == null) {
            ...
            updateRowData(start, data.getContents());
        } else {
    ...
    private boolean isContained(Range range) {
        if (data == null)
            return false;
        if (data.getContents().size() == data.getTotalElements())
            return true;
        if ((range.getStart() + range.getLength()) > (start + data.getContents().size()) ||
    ...
    public boolean hasMore() {
        if (data == null || data.getContents().size() < data.getTotalElements())
            return true;
        return false;
    }
    ...
    private void clientSideFiltering() {
        if (!passportProxyFilter.isDirty())
            return;
        filteredView = ImmutableList.copyOf(Collections2.filter(data.getContents(), Predicates.and(passportProxyFilter.getPredicates())));
    }
    ...
    public void setPassportProxyFilter(PassportListPresenter.PassportProxyFilter passportProxyFilter) {

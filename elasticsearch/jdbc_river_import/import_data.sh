...
curl -XPUT $es_host/_river/${index}_phenotype_river/_meta -d "{
    ...
    \"jdbc\" : {
        ...
        \"user\" : \"$db_user\",
        \"password\" : \"$db_password\",
        \"sql\" : \"SELECT DISTINCT div_trait_uom_id as _id,div_experiment_id as _parent, local_trait_name,trait_protocol,unit_type as \\\"div_unit_of_measure.unit_type\\\", published, modified,created,permission as \\\"acl.permissions\\\", permission_id as \\\"acl.id\\\",owner_id as \\\"owner.id\\\",owner_sid as \\\"owner.sid\\\", owner_name as \\\"owner.name\\\",experiment as \\\"experiment.name\\\",div_experiment_id as \\\"experiment.id\\\",t_ont.acc as \\\"to_accession.term_id\\\",t_ont.name as \\\"to_accession.term_name\\\",t_ont.definition as \\\"to_accession.term_definition\\\",t_ont.comment as \\\"to_accession.term_comment\\\",eo_ont.acc as \\\"eo_accession.term_id\\\",eo_ont.name as \\\"eo_accession.term_name\\\",eo_ont.definition as \\\"eo_accession.term_definition\\\",eo_ont.comment as \\\"eo_accession.term_comment\\\" FROM phenotype.view_search_div_trait_uom_acl LEFT JOIN dblink('dbname=Ontologies user=$db_user password=$db_password','SELECT acc,name,term_definition as definition,term_comment as comment FROM term LEFT JOIN term_definition ON term_definition.term_id = term.id') AS t_ont(acc varchar(255),name varchar(255),definition text,comment text) ON t_ont.acc = to_accession LEFT JOIN dblink('dbname=Ontologies user=$db_user password=$db_password','SELECT acc,name,term_definition as definition,term_comment as comment FROM term LEFT JOIN term_definition ON term_definition.term_id = term.id') AS eo_ont(acc varchar(255),name varchar(255),definition text,comment text) ON eo_ont.acc = eo_accession\"
   },
    \"index\":{
       \"index\":\"$index\",
       \"type\":\"phenotype\"

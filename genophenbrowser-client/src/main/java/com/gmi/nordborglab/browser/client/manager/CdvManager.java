...
public class CdvManager extends RequestFactoryManager<CdvRequest> {
    ...
    public void findStudiesByPhenotypeId(Receiver<StudyPageProxy> receiver, ConstEnums.TABLE_FILTER filter, String searchString, Long id, int start, int size) {
        getContext().findAll(id, filter, searchString, start, size).with("contents.alleleAssay", "contents.protocol", "contents.job", "facets", "contents.ownerUser", "contents.transformation").fire(receiver);
    }
    ...
    public void findAll(Receiver<StudyPageProxy> receiver, ConstEnums.TABLE_FILTER filter, String searchString, int start, int size) {
        getContext().findAll(filter, searchString, start, size).with("contents.alleleAssay", "contents.protocol", "contents.phenotype.experiment", "contents.job", "facets", "contents.ownerUser", "contents.transformation").fire(receiver);
    }

...
public class SNPDetailPresenterWidgetView
        extends ViewWithUiHandlers<com.gmi.nordborglab.browser.client.mvp.widgets.snps.SNPDetailUiHandlers> implements SNPDetailPresenterWidget.MyView {
    ...
    interface Binder extends UiBinder<Widget, SNPDetailPresenterWidgetView> {
    ...
    private final IdentityColumn<SNPAllele> phenotypeColumn;
    ...
    @Inject
    SNPDetailPresenterWidgetView(final Binder binder, final CustomDataGridResources customDataGridResources,
        ...
        phenotypeColumn = new IdentityColumn<SNPAllele>(new SNPDetailDataGridColumns.PhenotypeCell(cells));
    ...
    private void initDataGrid(FlagMap flagMap) {
        ...
        alleleDataGrid.addColumn(phenotypeColumn, new TextHeader("Phenotype"));
    ...
    private double calculateMean(Collection<SNPAllele> snpAlleles) {
        ...
        for (SNPAllele snpAllele : snpAlleles) {
            if (snpAllele.getPhenotype() != null)
        ...
        ImmutableListMultimap<String, SNPAllele> countries = Multimaps.index(snpAlleles, new Function<SNPAllele, String>() {
            ...
            public String apply(SNPAllele input) {
                ...
                if (input.getPassport() != null
                        ...
                        && input.getPassport().getCollection().getLocality() != null
                        && input.getPassport().getCollection().getLocality().getCountry() != null) {
                    country = input.getPassport().getCollection().getLocality().getCountry();
                }
                return country;
            }
        });
    ...
    public void displayAlleInfo(SNPInfoProxy snpAnnot) {
        ...
        chrLb.setInnerText(snpAnnot.getChr());
        posLb.setInnerText(String.valueOf(snpAnnot.getPosition()));
    }
    ...
    @Override
    public void showPhenotypeColumns(boolean show) {
        if (!show) {
            alleleDataGrid.setColumnWidth(phenotypeColumn, "0px");
            updateChartType(CHART_TYPE.table);
        } else {
            alleleDataGrid.setColumnWidth(phenotypeColumn, null);
        }
        boxplotChartBtnContainer.setVisible(show);
        hasPhenotypes = show;
